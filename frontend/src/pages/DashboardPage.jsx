...
import { BatteryCharging, Zap, MapPin, DollarSign } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      <div>
        <h1 style={{ fontSize: '22px', fontWeight: 800, margin: '0 0 4px', color: '#151A2D' }}>Halo, {user?.name || 'Ilham'}</h1>
        <p style={{ fontSize: '13px', color: '#6B7280', margin: 0 }}>Kelola energi kendaraan Anda dan dapatkan manfaat dari V2G.</p>
      </div>

      <div className="grid-4">
        {[
          { label: 'Status V2G', value: 'Layak V2G', type: 'badge', color: '#16A34A', bg: '#E9F8EF' },
          { label: 'Energi tersedia untuk V2G', value: '8.4 kWh', type: 'num' },
          { label: 'Estimasi pendapatan hari ini', value: 'Rp 16.800', type: 'num' },
          { label: 'Saldo kompensasi', value: 'Rp 42.500', type: 'num' },
        ].map((s) => (
          <div key={s.label} style={{ background: '#fff', border: '1px solid #E4E7EE', borderRadius: '14px', padding: '18px' }}>
            <div style={{ fontSize: '12px', color: '#6B7280', marginBottom: '6px', fontWeight: 500 }}>{s.label}</div>
            {s.type === 'badge' ? (
              <span style={{ display: 'inline-block', fontSize: '12px', fontWeight: 700, padding: '4px 10px', borderRadius: '100px', background: s.bg, color: s.color }}>{s.value}</span>
            ) : (
              <div style={{ fontSize: '20px', fontWeight: 800, color: '#151A2D' }}>{s.value}</div>
            )}
          </div>
        ))}
      </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E4E7EE', borderRadius: '14px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#98A1B0', marginTop: '2px' }}>EV-001 (Hyundai Ioniq 5)</div>
            </div>
            <span style={{ fontSize: '11px', fontWeight: 700, padding: '4px 10px', borderRadius: '100px', background: '#EAF0FF', color: '#2F5AF7' }}>85% SOC</span>
          </div>
          <p style={{ fontSize: '13px', color: '#6B7280', margin: '0 0 14px', lineHeight: 1.6 }}>Terhubung ke charger, aman digunakan untuk V2G hingga 07:00 besok.</p>
        ...
        <div style={{ background: '#fff', border: '1px solid #E4E7EE', borderRadius: '14px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#98A1B0', marginTop: '2px' }}>Stasiun Wonokromo</div>
            </div>
          </div>
          <p style={{ fontSize: '13px', color: '#6B7280', margin: '0 0 14px', lineHeight: 1.6 }}>350 m dari lokasi Anda. Tarif Rp 2.000/kWh, 4/6 connector tersedia.</p>
...
function DashboardPengelola() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      <div>
        <h1 style={{ fontSize: '22px', fontWeight: 800, margin: '0 0 4px', color: '#151A2D' }}>Dashboard</h1>
        <p style={{ fontSize: '13px', color: '#6B7280', margin: 0 }}>Ringkasan kondisi baterai dan aktivitas V2G seluruh armada.</p>
      </div>
      <div className="grid-4">
        {[
